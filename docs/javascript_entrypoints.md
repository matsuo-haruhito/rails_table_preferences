...
| Lifecycle events | Copied-controller registrations do not receive the package-entrypoint lifecycle event surface unless the host app ports that behavior into its copied or replacement controller. | Dispatches package-entrypoint lifecycle events such as `applied`, `saved`, `loaded`, `deleted`, and `error` with stable detail payloads, including documented `error` action labels. | Check host-app event listeners against the registration path that actually owns the screen. Use JavaScript controller notes for the `error` action list. |
