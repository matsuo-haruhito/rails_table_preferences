...
| Controller or bundled editor UI | Open a representative editor screen, apply one visibility/order change, and check desktop plus one narrow width for clipping, focus visibility, and helper/status copy. For filter panel changes, use a short viewport with a multi-select or several-field panel and confirm Apply / Clear remain reachable. | Screenshot, browser notes, focused system spec, or DOM evidence for desktop/narrow checks; for filter panels, record the viewport height, representative field type, and how Apply / Clear were reached. | The PR changes async preset actions, drag/resize/filter/sort interactions, accepted keyboard behavior, filter panel positioning, or filter panel action reachability. |
