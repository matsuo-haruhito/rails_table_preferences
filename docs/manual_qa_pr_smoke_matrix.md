...
| UI, CSS, accessibility, or high-contrast state | Use [Accessibility baseline](accessibility.md#forced-colors-and-high-contrast-checks) as the source of truth and check the representative states touched by the PR: active filter buttons, sorted headers, focused resize handles, filter panel boundaries, and pinned/fixed columns. | Browser notes, screenshot, or browser-capable handoff for the forced-colors / high contrast mode used, representative states checked, skipped states and reason, and any host-app theme responsibility that remains outside the gem. | The PR changes runtime CSS, color tokens, focus styles, filter panel layering, pinned/fixed column styling, or claims rendered accessibility evidence that cannot be checked in the current environment. |
