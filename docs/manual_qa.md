...
- [ ] If you use the generated demo example, set `scope_context_method` to return `organization: "tokyo-hq"` and confirm `本部ビュー [organization:tokyo-hq]` appears.
...
- [ ] In the generated demo flow, clear any owner default and confirm `担当ビュー [role:operations]` wins before `本部ビュー [organization:tokyo-hq]` and `共有ビュー [shared]`.
- [ ] In the generated demo flow, temporarily remove the role key and confirm `本部ビュー [organization:tokyo-hq]` wins before `共有ビュー [shared]`.
