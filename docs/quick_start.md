...
If the same page renders more than one `table_preferences_editor`, pass `editor_instance_key:` when you want deterministic label/input ids, especially for copied or customized partials.

```erb
<%= table_preferences_editor(
  table_key: :orders,
  name: "default",
  editor_instance_key: "orders-inline",
  columns: @table_columns,
  title: "画面内の設定"
) %>
...
- Use `editor_instance_key` only to distinguish editor instances on the same page, or when copied views need predictable ids for labels, tests, or wrapper markup.
- Use a stable string such as `orders-inline`, `orders-dialog`, or another view-level identifier, not a random value generated by the host app.
- In copied/customized partials, preserve the label `for` to preset select/name `id` pairing instead of hard-coding duplicate ids.

If the host app renders only one editor for the page, you can usually omit `editor_instance_key:` and keep the default behavior.
