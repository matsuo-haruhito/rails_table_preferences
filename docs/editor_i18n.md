...
In the current bundled editor, the preset selector is the source for loading or switching saved settings, while the preset name input is the destination name used by save and save as new. If a user selects an existing preset, changes the name input, and saves, the bundled editor treats the typed name as the save target; it does not present that action as an in-place rename of the previously selected preset. Keep host-app copy close to that boundary when overriding the selector hint, name hint, or action hint.
