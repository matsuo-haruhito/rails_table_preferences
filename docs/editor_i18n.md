...
`select_filter_option_search_label` and `select_filter_option_search_placeholder` control the package entrypoint's search input for long select-filter option lists. The bundled controller combines the current filter value label with `select_filter_option_search_label` for the generated `aria-label`, and uses `select_filter_option_search_placeholder` for the input placeholder. Override the locale keys globally, or override `data-rails-table-preferences-select-filter-option-search-label-value` and `data-rails-table-preferences-select-filter-option-search-placeholder-value` on one controller root when only one mounted table needs different wording.
...
These package entrypoint root values are provided by the package entrypoint subclass. Host apps that copy or register the base generated controller directly should not assume `filterOperatorLabels`, `selectFilterOptionSearchLabel`, or `selectFilterOptionSearchPlaceholder` is present; use copied or replacement JavaScript when changing controller behavior, adding new operators, or supporting a controller path that does not include the package entrypoint subclass. See [JavaScript entrypoints](javascript_entrypoints.md) before relying on package-only controller behavior from a copied or host-owned controller path.
...
ja:
  rails_table_preferences:
    editor:
      ...
      filter: 条件で絞り込む
      select_filter_option_search_label: 選択肢を検索
      select_filter_option_search_placeholder: 選択肢名で絞り込み
      filter_operator_labels:
        contains: 含める
        equals: 完全一致
      sort_asc: 昇順で並び替え
...
2. Controller-root `data-rails-table-preferences-*-label-value` attributes when one mounted table needs different filter, sort, select option search, or scope fallback wording.
