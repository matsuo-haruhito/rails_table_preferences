...
The images below stay intentionally lightweight, but they now mirror the current generated demo more closely: Japanese labels, the seeded shared preset `共有ビュー`, practical search/sort cues, and the same dense list-screen posture used elsewhere in the docs. Use the generated demo screen as the behavioral source of truth and treat these images as a compact orientation aid rather than a full QA substitute.

## Editor and shared preset flow

![Bundled demo-aligned view showing the shared preset path, scoped preset orientation, and the lightweight orders screen.](images/visual-overview-editor-and-table.svg)
...
- Start with [Demo screen generator](demo.md) when you want the current scope context summary, scoped preset examples, and export payload preview that extend the screenshots on this page.
